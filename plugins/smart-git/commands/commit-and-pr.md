...
**重要**: コミットメッセージは必ず **1行形式** で実行してください。複数行のメッセージ（body）は生成しないでください。
...
**禁止事項**:
- HEREDOCや複数の `-m` オプションを使った複数行メッセージ
- 改行を含むメッセージ

コミット後、`git log -1` でコミットが正しく作成されたことを確認
...
PR作成前に、生成した内容をプレビュー表示してユーザーに確認を取ってください。
...
- 選択肢：
  - 「作成する」→ 次のステップへ進む
  - 「タイトルを修正」→ 新しいタイトルを入力してもらい、再度プレビュー表示
  - 「説明文を修正」→ 修正内容を入力してもらい、再度プレビュー表示
  - 「キャンセル」→ PR作成を中止（コミットは維持）
